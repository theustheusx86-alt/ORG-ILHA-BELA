...
function buildFilaEmbed(mode, plat, value) {
  ...
  return new EmbedBuilder()
    .setColor(plat==='mobile' ? 0x3498DB : plat==='misto' ? 0x2ECC71 : 0x9B59B6)
...
client.on('messageCreate', async (msg) => {
  ...
  if (cmd === 'fila') {
    ...
    if (!Number.isFinite(value) || value <= 0) return msg.reply(`❌ Valor inválido. Ex: !fila 1v1 mobile 200,00`);
    try {
      const k = getFilaKey(mode,plat,value);
